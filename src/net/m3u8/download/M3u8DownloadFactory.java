...
public class M3u8DownloadFactory {
    ...
    public static class M3u8Download {
        ...
        private void startDownload() {
            ...
            //start multiple thread download
            for (String s : tsSet) {
                i++;
                System.out.println(s);
                fixedThreadPool.execute(downloadRunnable(s, i));
            }
            ...
            new Thread(() -> {
                int consume = 0;
                //asking if the download is complete
                ...
                while (!fixedThreadPool.isTerminated()) {
                    try {
                        ...
                        for (DownloadListener downloadListener : listenerSet) {
                            ...
                            int sum = tsSet.size();
                            BigDecimal finishedCountInBD = new BigDecimal(finishedCount);
                            BigDecimal totalInBD = new BigDecimal(tsSet.size());
                            System.out.println("finished Count:" + finishedCountInBD);
                            System.out.println("total size:" + totalInBD);
                            float percent = finishedCountInBD
                                    .divide(totalInBD, 4, BigDecimal.ROUND_HALF_UP)
                                    .multiply(new BigDecimal(100)).setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();
                            downloadListener.process(DOWNLOADURL, finished, sum, percent);
         ...
         * @return thread
         */
        private Runnable downloadRunnable(String urls, int i) {
            Runnable runnable;
                ...
                public void run() {
                    ...
                    while (count <= retryCount) {
                        try {
                            ...
                            int len;
                            System.out.println("将未解密的ts片段写入文件: " + dir + FILESEPARATOR + i + ".xy");
                            //将未解密的ts片段写入文件
...
//                Log.i(urls + "下载完毕！\t已完成" + finishedCount + "个，还剩" + (tsSet.size() - finishedCount) + "个！");
        ...
        private String getTsUrl() {
            ...
            boolean isKey = false;
            for (String s : split) {
                //If hv the below pattern, means only one layer of m3u8
                if (s.contains("#EXT-X-KEY") || s.contains("#EXTINF")) {
                    isKey = true;
                    keyUrl = DOWNLOADURL;
                    break;
                }
                //If hv the below pattern, means more than one layer of m3u8
                if (s.contains(".m3u8")) {
        ...
        /**
         * Get Key && Add ts_url into  Set<String> tsSet
         *
         * @param url     Key URL
         * @param content string
         * @return if the ts need to decry,
        ...
        private String getKey(String url, StringBuilder content) {
            ...
            if (!StringUtils.isEmpty(key)) {
                key = key.replace("\"", "");
                String key_url = StringUtils.isUrl(key) ? key : mergeUrl(relativeUrl, key);
                return getUrlContent(key_url, true).toString().replaceAll("\\s+", "");
            }
            return null;
        }

        /**
         * Like Http Request and get return
         *
         * @param urls  http url
         * @param isKey if the url is use to get key??
